...
async def givename(session: nonebot.CommandSession):
    ...
    if not await session.send(f'[CQ:image,file={setu_link}]'):
        setu_link = setu_link.split('.jpg')[0] + '-1.jpg'
        await session.send(f'[CQ:image,file={setu_link}]')
    print(f'[setu] : {setu_link}')
    ...
    async with aiohttp.ClientSession() as session:
        while(1):
            async with session.get(link, params=data) as response:
                ...
                return (f'https://pixiv.cat/{pic_id}.jpg')
